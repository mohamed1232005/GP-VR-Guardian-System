...
class AIPipelineWorker:
    ...
    def _run_pipeline(self, frame, metadata, frame_id) -> dict:
        ...
        # ================================================================
        # Phase 1 [CONTRACT_FIX]: GEOMETRY pose = M_col (TRUE pose).
        #
        # Unity serialises camera_to_world COLUMN-MAJOR (see Unity
        # AIFramePacket.SetCameraToWorld), and consumes the boundary as
        #   world = framePose.position + framePose.rotation * camera_point
        # i.e. with that SAME column-major matrix. The horizontal-score heuristic
        # above may select M_row (the TRANSPOSE) — fine for the accept/reject
        # gate, but if we build the WORLD rectangle and map it back with the
        # transpose, Unity re-applies the true pose and the cube comes back
        # rotated up onto the wall (the reported vertical-grid bug). So ALL world
        # geometry (accumulation + the corners we transmit) uses M_col only.
        M_geom = M_col
        # Floor normal in the M_col world frame, using the POINTS convention
        # (flip Y only — identical to boundary_generator / _world_space_check),
        # so the accumulator's plane-distance band is consistent with M_geom.
        normal_cam_pts   = np.array([normal_cv[0], -normal_cv[1], normal_cv[2]], dtype=np.float64)
        normal_world_geom = _normalize(R_col @ normal_cam_pts)
        if normal_world_geom[1] < 0.0:
            normal_world_geom = -normal_world_geom
        dbg["normal_world_geom"] = [round(v, 4) for v in normal_world_geom.tolist()]
        print(f"[PY_GEOM_POSE] order=col(forced) "
              f"normal_world_geom=({normal_world_geom[0]:.3f},"
              f"{normal_world_geom[1]:.3f},{normal_world_geom[2]:.3f}) "
              f"selected_for_gate={selected_order}")
        ...
        # ---- Lock decision: world accumulation stable (req 8) ----
        if lock_allowed and self._floor_accum.ready_to_lock():
        ...
        # ---- Not locked yet: preview the accumulating FLAT cube ----
        if not lock_allowed:
            print(f"[P7_CANDIDATE] id={frame_id} lock_allowed=False "
                  f"reason={lock_block_reason}")
