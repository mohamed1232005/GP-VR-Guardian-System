...
def main():
    ...
    # Phase 10.2 (J): declare early so shutdown() closure always sees the name.
    unity_lock_listener = None
    ...
    def shutdown(signum, frame):
        ...
        udp_receiver.stop()
        tcp_server.stop()
        # Phase 10.2 (J): cleanly close the Unity lock listener if it started.
        # Closure reads outer-scope variable; default is None when it never started.
        try:
            ull = unity_lock_listener  # may be None
            if ull is not None:
                ull.stop()
        except NameError:
            pass  # listener was never declared (init failed before this line)
    ...
    # Phase 10.2 (J): start the Unity→Python lock-event listener on a separate
    # port. It records the lock_frame_id Unity actually accepted so session
    # metrics no longer show "Lock Frame None". Failure to start is non-fatal —
    # the guardian pipeline still works without it. unity_lock_listener was
    # pre-declared as None above so shutdown() can reference it safely.
    try:
        from networking.unity_lock_listener import UnityLockListener
        unity_lock_listener = UnityLockListener(port=9991)
        if not unity_lock_listener.start():
            unity_lock_listener = None
    except Exception as e:
        print(f"[U_LOCK_LISTEN_INIT_ERR] {type(e).__name__}: {e}")
        unity_lock_listener = None
    ...
    if pipeline_worker is not None:
        try:
            ...
            # Phase 10.2: hand listener to the pipeline so it can stamp metrics.
            if unity_lock_listener is not None and hasattr(pipeline_worker, "set_unity_lock_listener"):
                pipeline_worker.set_unity_lock_listener(unity_lock_listener)
