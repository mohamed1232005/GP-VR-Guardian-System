...
class FloorAccumulator:
    ...
    def __init__(
        ...
        rolling_lock: bool = True,
        lock_max_wait_frames: int = 25,
        auto_lock_min_area_m2: float = 1.5,
    ):
        self._area_growth_frames = int(area_growth_frames)
        self._area_growth_eps    = float(area_growth_eps_m2)
        self._rolling_lock       = bool(rolling_lock)
        self._lock_max_wait      = int(lock_max_wait_frames)
        self._auto_lock_min_area = float(auto_lock_min_area_m2)
    ...
    def stability(self):
        ...
        # PLACEMENT stability = where the cube SITS (center), how it is ORIENTED
        # (yaw) and how HIGH the floor is (floorY). This is independent of the
        # cube's SIZE, which is allowed to keep growing while the user scans more
        # floor. Rolling lock keys off placement so a still-growing-but-steadily-
        # placed rectangle can lock and capture the full area.
        place_stable = (
            d_center <= self._center_tol and
            d_yaw    <= self._yaw_tol and
            d_floory <= self._floory_tol
        )
        # FULL stability additionally requires size to have settled (legacy gate).
        stable = (
            place_stable and
        ...
        return {
            "stable": bool(stable), "place_stable": bool(place_stable), "frames": n,
            "d_center": d_center, "d_yaw": d_yaw, "d_floory": d_floory,
            "d_w": d_w, "d_d": d_d,
        }

    def _meets_minimums(self) -> bool:
        """Safe floor minimums shared by auto AND manual lock: enough accepted
        frames, enough occupied cells, and a big-enough rectangle."""
        return (not self._locked
                and self._last_rect is not None
                and self._frame_count >= self._min_frames
                and self.cell_count >= self._min_cells
                and self._last_rect["area"] >= self._min_area)

    def can_lock_now(self) -> bool:
        """Req 4: minimums for a MANUAL (user-pressed) lock — the safe floor
        thresholds WITHOUT the area-growth / stability WAIT. The caller still
        applies the per-frame lock gate + Unity-space coordinate sanity."""
        return self._meets_minimums()

    def ready_to_lock(self) -> bool:
        """Auto-lock readiness — favours the LARGEST trusted area without stalling.

        Spec: auto-lock when placement (center+yaw+floorY) is steady AND the area
        is at least AUTO_LOCK_MIN_AREA. To capture the WHOLE room we keep the yellow
        preview growing while the trusted area is still actively expanding, then
        lock the largest-so-far rectangle once growth pauses (a short
        area_growth_frames plateau) — or, as an absolute anti-stuck backstop, after
        lock_max_wait accepted frames. Width/depth need NOT be perfectly stable
        (placement is enough); the manual Lock-Now path covers any remaining case.

        LEGACY mode: the old strict gate (area plateaued AND full size stability)."""
        if not self._meets_minimums():
            return False
        stab = self.stability()
        if self._rolling_lock:
            # placement steady (where it sits / how it's oriented / floor height)
            if not stab.get("place_stable", False):
                return False
            # usable area — never auto-lock a tiny partial patch
            if self._last_rect["area"] < self._auto_lock_min_area:
                return False
            # PREFER LARGEST: while the area is still growing, keep previewing
            # (yellow). Lock once growth has paused for area_growth_frames, OR the
            # absolute backstop is hit so a perpetually-growing scan still locks.
            still_growing = self._growth_stable_frames < self._area_growth_frames
            if still_growing and self._frame_count < self._lock_max_wait:
                return False
            return True
        # legacy strict gate
        if self._growth_stable_frames < self._area_growth_frames:
            return False
        return stab.get("stable", False)
