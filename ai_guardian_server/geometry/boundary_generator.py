...
def generate_boundary(
    ...
    **kwargs,
    ...
    x_lo = float(np.percentile(xs_clean, 5))
    x_hi = float(np.percentile(xs_clean, 95))
    z_lo = float(np.percentile(zs_clean, 5))
    z_hi = float(np.percentile(zs_clean, 95))
    ...
    min_w = max(FLOOR_CANDIDATE_MIN_W, 1.4)   # Phase 9.3: enforce 1.4m minimum
    min_d = max(FLOOR_CANDIDATE_MIN_D, 1.4)   # Phase 9.3: enforce 1.4m minimum
    max_w = min(FLOOR_CANDIDATE_MAX_W, 3.0)
    max_d = min(FLOOR_CANDIDATE_MAX_D, 3.0)
    ...
    for i, (scale, label) in enumerate(zip(scales, labels)):
        ...
        corner_coverage = _check_corner_coverage(corners, xs_clean, zs_clean, radius=0.2)
    ...
    for idx in range(len(candidates) - 1, -1, -1):
        if candidates[idx]["corner_coverage"] >= 0.5:
            best_idx = idx
            break
