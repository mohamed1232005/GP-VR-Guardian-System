...
#
# ENABLED BY DEFAULT (Phase 2+3): the body-pose pipeline (laptop webcam -> MediaPipe
# Pose) runs on startup so Unity's Toshfa rehab can consume raw BODY_POSE landmarks.
# Python stays stateless CV here too -- it only emits raw landmarks; Unity owns all
# rehab interpretation (pose matching / holds / progression).
#
# Override for weak hosts: GUARDIAN_POSE_ENABLED=0 turns the whole pipeline off, so
# server.py never opens the webcam and never starts the pose capture/worker threads,
# the hand pipeline gets the full CPU, and Unity falls back to "body tracking
# unavailable". pose_landmarker.task must sit beside server.py; if it is missing the
# pose threads are skipped at startup and hand tracking keeps running (no crash).
POSE_ENABLED = _env_flag("GUARDIAN_POSE_ENABLED", True)
# Webcam device index for the pose pipeline. Override with GUARDIAN_POSE_CAMERA_INDEX
# (e.g. =1) if camera 0 is the wrong device.
POSE_CAMERA_INDEX = _env_int("GUARDIAN_POSE_CAMERA_INDEX", 0)
# Optional laptop-side debug preview window (cv2.imshow) drawing the live webcam frame
# plus the MediaPipe Pose skeleton. OFF by default (headless server). Enable with
# GUARDIAN_POSE_PREVIEW=1. Preview is laptop-only debugging; the webcam image is never
# streamed to Unity. Press 'q' to close the window without stopping the server.
POSE_PREVIEW = _env_flag("GUARDIAN_POSE_PREVIEW", False)
POSE_TARGET_FPS = 15
